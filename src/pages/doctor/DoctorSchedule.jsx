...
export default function DoctorSchedule() {
  ...
  return <div className="doctor-schedule-page">
    <header><h1>My Schedule</h1><p>View appointments and manage your bookable availability.</p></header>
    ...
    </section><aside className="schedule-panel"><h2>Day Summary</h2><p>{formatBookingDate(date)}</p><dl><div><dt>Total appointments</dt><dd>{appointments.length}</dd></div><div><dt>Completed</dt><dd>{appointments.filter(item => item.status === 'completed').length}</dd></div><div><dt>Pending or confirmed</dt><dd>{appointments.filter(item => ['pending', 'confirmed'].includes(item.status)).length}</dd></div></dl><p>Select an appointment to view the assigned patient’s details.</p></aside></div></> : <DoctorAvailabilityManager />}
  </div>;
}
