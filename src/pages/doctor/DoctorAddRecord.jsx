...
function RecordForm({ id, selection }) {
  ...
  if (saved) return <div className="doctor-record-page"><h1>Medical Record Saved</h1><RecordSection title="Consultation recorded"><p role="status">The medical record and {saved.prescriptions.length} prescription(s) were saved in this mock preview. This appointment cannot receive a duplicate record.</p><p><strong>{saved.diagnosis}</strong></p><p>Return to Patient Details to review and mark the consultation completed. Mock data is kept in memory until the application reloads.</p><div className="record-actions">{back}<Link className="action-link primary-action" to={`/doctor/records/${saved.id}/certificate/new`}>Issue Medical Certificate</Link></div></RecordSection></div>;
