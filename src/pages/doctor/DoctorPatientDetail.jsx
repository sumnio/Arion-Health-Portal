...
export default function DoctorPatientDetail() {
  ...
  const { patient, appointment, history, existingRecord, canAddRecord, consultationMessage } = detail;
  return <div className="doctor-patient-page">{back}
    ...
    <div className="doctor-patient-grid"><div>
      ...
      <RecordSection title="Previous Medical Records">{history.length ? <ul className="clinical-history">{history.map(record => <li key={record.id}><time dateTime={record.encounter_at}>{formatBookingDate(record.encounter_at.slice(0, 10))}</time><p>{record.doctor}</p><strong>{record.diagnosis}</strong></li>)}</ul> : <p>No previous medical records available.</p>}</RecordSection>
    </div><div>
