  ...
  return <div className="staff-patients">
    ...
    <section className="patients-panel" aria-label="Patient directory">
      ...
      {!result.items.length ? <div className="patients-empty"><h2>{result.total ? 'No matching patients' : 'No patients yet'}</h2><p>{result.total ? 'Try another name or contact number.' : 'Register a new walk-in patient to get started.'}</p>{query && <button onClick={() => { setQuery(''); setPage(1); }}>Clear search</button>}</div> : <ul className="patients-list">{result.items.map(patient => <li key={patient.id} className="patient-card">
        <div className="patient-card-heading"><h2>{patient.full_name}</h2><Link className="action-link" aria-label={`Create Walk-in Appointment for ${patient.full_name}`} to={`/staff/patients/${patient.id}/walk-in`}>Create Walk-in Appointment</Link></div>
        <dl><div><dt>Date of birth / age</dt><dd>{patient.dob ?? 'Not available'}{patient.age !== null && ` · ${patient.age} years`}</dd></div><div><dt>Sex</dt><dd>{patient.sex || 'Not available'}</dd></div><div><dt>Contact number</dt><dd>{patient.contact_number || 'Not available'}</dd></div><div><dt>PWD</dt><dd>{patient.is_pwd ? 'Yes' : 'No'}</dd></div><div><dt>Senior (derived from DOB)</dt><dd>{patient.isSenior === null ? 'Not available' : patient.isSenior ? 'Yes' : 'No'}</dd></div></dl>
      </li>)}</ul>}
