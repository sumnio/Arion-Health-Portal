...
export default function StaffRegisterWalkIn() {
  ...
  const [values, setValues] = useState({ full_name: '', dob: '', sex: '', contact_number: '', address: '', emergency_contact_name: '', emergency_contact_number: '', emergency_contact_relationship: '', allergies: '', is_pwd: false });
  ...
  function results(items) {
    return <ul className="walkin-results">{items.map(item => <li key={item.id}><div><strong>{item.full_name}</strong><p>{item.dob} · {item.contact_number}</p></div><Link className="action-link" to={`/staff/patients/${item.id}/walk-in`}>Select patient</Link></li>)}</ul>;
  }
  ...
  return <div className="staff-walkin"><Link to="/staff/dashboard">← Back to Dashboard</Link><h1>Register Walk-in</h1><p>Find an existing patient first, or register a patient without a portal account.</p><div className="walkin-layout"><div>
    ...
    <form className="walkin-panel" onSubmit={submit} noValidate><h2>2. New walk-in patient</h2><p>Fields marked * are required. No email, password, or portal account is needed.</p>
      ...
      <div className="walkin-fields">{field('full_name', 'Full name *')}{field('dob', 'Date of birth *', 'date')}{field('sex', 'Sex *', 'text', <><option value="">Select sex</option>{profileSexOptions.map(value => <option key={value}>{value}</option>)}</>)}{field('contact_number', 'Contact number *', 'tel')}{field('address', 'Address (optional)')}{field('emergency_contact_name', 'Emergency contact name (optional)')}{field('emergency_contact_number', 'Emergency contact number (optional)', 'tel')}{field('emergency_contact_relationship', 'Emergency contact relationship (optional)')}{field('allergies', 'Allergies (optional, separated by commas)')}</div>
