...
export default function StaffCalendar() {
  ...
  function changeDate(value) {
    ...
    setDate(value); setSelectedId(null); setDoctor(''); setMessage(''); setError(''); setCancelling(false);
  }
  ...
  return <div className="staff-calendar">
    ...
    <div className="calendar-controls">
      ...
      <label>Doctor<select value={doctor} onChange={event => setDoctor(event.target.value)}><option value="">All doctors</option>{doctors.map(([id, name]) => <option key={id} value={id}>{name}</option>)}</select></label>
      <label>Status<select value={status} onChange={event => setStatus(event.target.value)}><option value="">All statuses</option>{statuses.map(value => <option key={value} value={value}>{value === 'no_show' ? 'No-show' : value[0].toUpperCase() + value.slice(1)}</option>)}</select></label>
    </div>
