...
import { appointmentStatus, appointmentCheckIn, setAppointmentStatus } from '../mocks/staffAppointmentStore.js';
...
function present(item) {
  item = { ...item, status: appointmentStatus(item), check_in_at: appointmentCheckIn(item) };
  ...
  return { ...item, doctor: doctor?.name ?? (item.doctor_id === demoDoctor.id ? demoDoctor.display_name : 'Doctor unavailable'), specialty: doctor?.specialty,
...
export const appointmentService = {
  ...
  cancel(id) {
    ...
    if (!canManageAppointment(item ? present(item) : null)) return { error: 'This appointment can no longer be cancelled.' };
    setAppointmentStatus(item, 'cancelled');
    return { appointment: present(item) };
  },
  reschedule(id, date, time) {
    const item = appointmentStore.find(item => item.id === id);
    if (!canManageAppointment(item ? present(item) : null)) return { error: 'This appointment can no longer be rescheduled.' };
