...
export const doctorPatientService={
 get(id,selection,now=new Date()){
  ...
  const appointment=selection?.appointmentId?appointments.find(item=>item.id===selection.appointmentId&&item.patient_id===id&&item.appointment_at.slice(0,10)===selection.date):appointments.find(item=>item.patient_id===id&&item.appointment_at.slice(0,10)===today);
