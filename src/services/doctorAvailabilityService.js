...
import { slotOccupied } from '../mocks/appointmentStore.js';
...
export const doctorAvailabilityService = {
  ...
  setDayActive(day, active, doctorId = demoDoctor.id) {
  ...
  addRecurring({ day_of_week, start_time, end_time }, doctorId = demoDoctor.id) {
    ...
    const item = { id: crypto.randomUUID(), doctor_id: doctorId, day_of_week: day, start_time, end_time, is_active: true };
    doctorAvailabilityStore.push(item);
    return { recurring: clone(item) };
  },
  removeRecurring(id, doctorId = demoDoctor.id) {
  ...
  publish({ date, start_time, end_time }, doctorId = demoDoctor.id, now = new Date()) {
    ...
    const item = { id: crypto.randomUUID(), doctor_id: doctorId, date, start_time, end_time };
    doctorPublishedAvailabilityStore.push(item);
    return { published: clone(item) };
  },
  removePublished(id, doctorId = demoDoctor.id) {
  ...
  addBlocked({ date, whole_day, start_time, end_time, reason }, doctorId = demoDoctor.id, now = new Date()) {
    const window = this.publicationWindow(now);
    ...
    const item = { id: crypto.randomUUID(), doctor_id: doctorId, start_at, end_at, reason: reason.trim() };
    doctorBlockedTimeStore.push(item);
    return { blocked: clone(item) };
  },
  removeBlocked(id, doctorId = demoDoctor.id) {
  ...
  getSlots(doctorId, date, now = new Date()) {
    ...
    return ranges.flatMap(range => {
      ...
      for (let value = minutes(range.start_time); value + 30 <= minutes(range.end_time); value += 30) {
        ...
        const occupied = slotOccupied(doctorId, date, time);
