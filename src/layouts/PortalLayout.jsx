...
export default function PortalLayout({ role }) {
  ...
  return <div className={'portal-shell theme-' + role}>
    ...
    <header className="portal-header" onKeyDown={closeOnEscape}>
      {role === 'patient' && <button ref={menuButton} className="patient-menu-toggle" type="button" aria-label={menuOpen ? 'Close navigation' : 'Open navigation'} aria-expanded={menuOpen} aria-controls="patient-sidebar" onClick={() => setMenuPath(menuOpen ? null : pathname)}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" aria-hidden="true"><path d={menuOpen ? 'M6 6l12 12M6 18 18 6' : 'M4 6h16M4 12h16M4 18h16'} /></svg></button>}
    ...
    <aside id={role === 'patient' ? 'patient-sidebar' : undefined} className={'sidebar' + (menuOpen ? ' sidebar-open' : '')} onKeyDown={closeOnEscape} onClick={event => { if (event.target.closest('a')) setMenuPath(null); }}><Navigation label={role + ' navigation'} routes={routeGroups[role].filter(route => !route.path.includes(':id'))} /><Link className="exit-link" to="/login">Exit mock preview</Link></aside>
