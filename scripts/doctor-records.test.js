...
const sharedSelection = () => ({ appointmentId: '70000000-0000-4000-8000-000000000011', date: clinicToday() });
...
test('required fields and prescription rows are validated before writing', () => {
  const result = service.save(patient(1), sharedSelection(), { ...values, diagnosis: ' ', encounter_at: '', prescriptions: [{medicine:'',dosage:''}] });
  for (const key of ['diagnosis','encounter_at','medicine-0','dosage-0']) assert(result.errors[key]);
  assert(doctorPatientService.get(patient(1),sharedSelection()).canAddRecord);
});
test('save links prescriptions, preserves optional fields, updates consultation and prevents duplicates', () => {
  const result = service.save(patient(1), sharedSelection(), {...values,prescriptions:[{medicine:'Mock medicine',dosage:'As directed',instructions:''}]});
  assert.equal(result.record.appointment_id,sharedSelection().appointmentId);
  assert.equal(result.record.prescriptions[0].medical_record_id,result.record.id);
  assert.equal(result.record.notes,null);
  assert.equal(doctorPatientService.get(patient(1),sharedSelection()).existingRecord.id,result.record.id);
  assert.equal(doctorPatientService.get(patient(1),sharedSelection()).canAddRecord,false);
  assert(service.save(patient(1),sharedSelection(),values).errors.form);
