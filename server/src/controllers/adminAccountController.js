export function createAdminAccountController({ adminAccountService: service }) {
  return {
    ...
    async deactivateDoctor(request, response) { response.json({ doctor: await service.deactivateDoctor(request.params.doctorId) }); },
    async reactivateDoctor(request, response) { response.json({ doctor: await service.reactivateDoctor(request.params.doctorId) }); },
    ...
    async deactivateStaff(request, response) { response.json({ staff: await service.deactivateStaff(request.params.staffId) }); },
    async reactivateStaff(request, response) { response.json({ staff: await service.reactivateStaff(request.params.staffId) }); },
    ...
    async deactivatePatient(request, response) { response.json({ patient: await service.deactivatePatient(request.params.patientId) }); },
    async reactivatePatient(request, response) { response.json({ patient: await service.reactivatePatient(request.params.patientId) }); },
  };
}
